...
// it is not a good idea to use a reference velocity
double ref_v = 60;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Any addtions to the cost should be added to 'fg[0]'
    fg[0] = 0;
    size_t t;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //Set all non-acuators upper and lower limits
  ...
  predict_yvals.clear();
  for (i=0; i<N; i++) {
      predict_xvals.push_back(solution.x[x_start + i]);
      predict_yvals.push_back(solution.x[y_start + i]);
  }
